...
def display_header():
    """Display page title"""
    st.title("🔍 AI军事情报研究员(AI Military Researcher)")
    st.markdown("---")
...
def display_info_message():
    ...
    st.info(
        """
    **AI Military Researcher** is an intelligent research tool based on large language models, designed to assist users in conducting in-depth topic research on military and other fields.
...
def display_workflow():
    """Display AI Military Researcher architecture diagram"""
    import os
    
    with st.expander("🏗️ View System Architecture", expanded=False):
        # Path to the architecture diagram
        diagram_path = os.path.join(os.path.dirname(__file__), "assets", "architecture_diagram2.png")
        
        if os.path.exists(diagram_path):
            st.image(
                diagram_path,
                caption="AI Military Researcher System Architecture - Complete workflow from user input to final report generation",
            ...
            st.warning("⚠️ Architecture diagram not found. Run `python scripts/generate_architecture_diagram2.py` to generate it.")
            st.markdown(
                """
...
def display_footer():
    ...
    st.markdown(
        f"""
        <div class="footer">
            <p>© 2026 AI Military Researcher |
            Author: Innovation Center</p>
